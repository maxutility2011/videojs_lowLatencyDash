...
(function(window) {
  ...
  ['debug', 'autoplay', 'muted', 'minified', 'liveui', 'partial', 'url', 'type', 'keysystems', 'buffer-water', 'low-latency-dash'].forEach(function(name) {
  ...
  window.startDemo = function(cb) {
    ...
    stateEls['low-latency-dash'].addEventListener('change', function(event) {
      saveState();
        ...
        player = window.player = window.videojs(videoEl, {
          ...
          html5: {
            vhs: {
              ...
              experimentalLowLatencyDash: getInputValue(stateEls['low-latency-dash'])
            }
          }
        });
