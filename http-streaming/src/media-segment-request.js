...
const handleSegmentBytes = ({
  ...
  if (isLikelyFmp4MediaSegment(bytesAsUint8Array)) {
    ...
    // BoZ: transfer ownership of bytes to (ts -> fmp4) transmux worker
...
export const mediaSegmentRequest = ({
  ...
  // BoZ: "segment.map.bytes" (the content of the init segment) is empty, need to request init segment first.
  if (segment.map && !segment.map.bytes) {
    console.log('!!!!!! loading init segment: ' + segment.map.resolvedUri);
